...
- **性质声明（诚实）**：本轮 E2 实跑为 self-eval（同一模型既作答又评分，符合
  blind_protocol.md §1 的冒烟测试定位），**不得称为盲测基线**。
- **外部盲测首轮已执行（2026-09-12）**：R1 = gpt-5.6-luna（Codex 净室，仓库外隔离）·
  R2 = Doubao Seed evolving · 33 案分层抽样（SHA 34857e224fd0）——**红线 0、
  均分 17.88/18**（29×18+4×17）；校准审计：盲测 ≈ self-eval（+0.17）触发协议 §4
  隔离审计，expected 短语指纹 1/33 为案例输入自带词汇、非泄露，结论"无泄露证据"
  已入报告；回归门 REG-001 首轮未过（缺语用边界）→ 回灌修复 + 净室重跑 18/18 通过，
  **回归子集 10/10** ✓。详见 `evals/blind/report_20260912.md` 与 `evals/BLIND_TEST_GUIDE.md`。
...
9. ~~**外部盲测未执行**~~ **首轮已执行（2026-09-12，33 案）**：红线 0、均分
   17.88/18，校准审计结论"无泄露证据"；残留：全量 208 案盲测与第三方人类抽检为下一迭代（REG-001 回归门已闭环）。
