...
## 2026-09-12（六）· 官网星图移动端点选兼容修复

星图（canvas 实现）备注此前仅由 hover 触发，触屏设备 tap 后手指抬起即触发
pointerleave 清除 hover，备注无法查看。修复：新增点选锁定——tap 节点固定显示
备注（拖拽时跟随），再 tap 同一节点取消，tap 空白关闭；桌面 hover 行为不变，
说明文案同步更新。gh-pages commit 9595dab，JS 全块语法校验通过，线上已验证
生效（site 200，6 项新逻辑关键字全量在位）。

## 2026-09-12（六）· 安全处置：移除自跑诊断案例与自用演示文稿（工作树 + git 历史）

作者自跑的诊断案例（木铎诊断案例/，8 个文件）曾随 v1.0.1 提交误入公开仓库，
后续安全清理时遗漏。本轮处置：git filter-repo 对 main 与 v1.0.0 做路径级历史改写
（13 个提交重写，tag 重指 dd76020）并强推覆盖远程，仓库根目录与历史均已无该路径；
本地文件恢复保留（untracked，仅存本地，不再公开）；.gitignore 补 /木铎诊断案例/ 防再发。
Release v1.0.0 与官网不受影响。注意：旧 SHA 直接链接在 GitHub GC 前或仍可访问，
如有需要可联系 GitHub Support 清除缓存视图。

同日追加处置：作者自用的宣传演示文稿 muduo-v1-deck.html / muduo-v1-deck.pdf 亦属
私有材料——HTML 自 v1.0.1 起随仓库公开，PDF 曾存于历史中间提交并作为 Release 附件
发布。已同步清除：Release 附件删除并更新发布说明；filter-repo 对 main 与 v1.0.0
再次路径级改写并强推覆盖（v1.0.0 tag 现指 91d27d4）；两文件仅存本地（untracked +
ignored）。README、site 与官网均无 deck 引用，删除零断裂。
