...
13. **Reader Language（交付物读者语言，交付自包含）**：诊断交付物写给**诊断发起人**，不写给模型。面向用户的交付物中禁止出现内部工作词汇——adapter 字段名（title_role 等）、模式代码（M1–M7）、节点/反模式编号（AP-LF-xxx）、内部状态词（MEMORY_BASED/Re-outline/Pass 2）、英文置信标签（HIGH/MEDIUM/LOW）。每个被点名的机制、反模式、平台概念必须**内联完整解释**（中文名＋原名＋等级＋在本文如何起作用＋边界），禁止让用户"去查知识库/源码"。交付前执行**术语清查**并留痕。结构化交付默认使用批注式对照阅读器模板——规范、模板与自包含清单见 §13 与 `templates/DELIVERABLE_SPEC.md`（合法保留词与降级路径也在其中）。
...
| 13 Reader Language | 18 | analyze_content / longform_article_analysis（交付环节：自包含门禁＋术语清查＋阅读器模板 templates/diagnosis_reader_v1.html） |
...
17. **（全模式）研究诚信**：重要结论是否可溯源（Recommendation → Mechanism → Evidence → Source）？
    ...
    是否与证据强度匹配？（铁律 12）
18. **（交付类任务）Reader Language 自包含门禁**：面向用户的交付物是否零内部工作词汇？
    每个被点名的机制/反模式/平台概念是否内联了"中文名＋原名＋等级＋本文作用＋边界"？
    是否存在任何"去查知识库/源码"式指针？术语清查是否执行并留痕？
    结构化交付是否用了批注式对照阅读器模板且通过 §3 四条门禁？（铁律 13，
    清单见 templates/DELIVERABLE_SPEC.md §3）
...
## 13. 交付物规范（批注式对照阅读器 v1.0）

> 规范：`templates/DELIVERABLE_SPEC.md`（结构规范、自包含门禁、证据卡格式、Intake 提问、质量核对）
> 模板：`templates/diagnosis_reader_v1.html`（含 7 步创作指南注释）
> 参考实现：`木铎诊断案例/01.focus.reader.html`（2026-09-10 用户验收）

- **适用**：M1/M2/M7 诊断类任务的结构化交付（用户要 HTML/报告/结构化呈现时默认使用）；
  用户要轻量文本时降级为分层 Markdown，**自包含门禁不豁免**。
- **形态**：零依赖单 HTML——三视图（左右对照／原文批注／诊断报告）＋三色批注
  （红问题/黄待改进/绿资产）＋双向定位＋优先级筛选＋施工清单（localStorage）＋打印态。
- **Intake 前置**（衔接铁律 9）：诊断前确认目标定位／发布状态与数据／受众圈层／投入预算／禁忌；
  无表现数据时，涉及预测的结论自动封顶"中低置信"。
- **交付动作序列**：填充模板 → §3 自包含四条门禁 → 术语清查留痕 → 浏览器自检（三视图/
  定位/筛选/清单/打印/控制台）→ 交付。
