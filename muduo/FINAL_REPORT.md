...
- **外部盲测首轮已执行（2026-09-12）**：R1 = gpt-5.6-luna（Codex 净室，仓库外隔离）·
  ...
  已入报告；回归门 REG-001 未过（缺语用边界）→ 已回灌修复、单案重跑待执行。
  详见 `evals/blind/report_20260912.md` 与 `evals/BLIND_TEST_GUIDE.md`。
...
9. ~~**外部盲测未执行**~~ **首轮已执行（2026-09-12，33 案）**：红线 0、均分
   17.88/18，校准审计结论"无泄露证据"；残留：REG-001 回归门单案重跑待执行、
   全量 208 案盲测与第三方人类抽检为下一迭代。
